...
const fetchProjects = async () => {
    try {
      ...
      // Sort by date (newest first) and take only last 4 for mobile, 3 for desktop
      const sortedProjects = allProjects.sort((a, b) => 
        new Date(b.createdAt || b.date) - new Date(a.createdAt || a.date)
      );
      
      // Take 4 projects to show 4 on mobile, but we'll limit to 3 on desktop with CSS
          ...
          <div className="max-w-7xl mx-auto text-center">
            <div className="animate-pulse">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-8">
                {/* Show 4 skeleton items on mobile, 3 on desktop */}
                {[1, 2, 3, 4].map((item) => (
                  <div key={item} className={item === 4 ? "hidden md:block lg:hidden" : ""}>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="max-w-7xl mx-auto relative overflow-hidden">
            ...
            <div className="text-center mb-16">
              <h1 className="text-lg md:text-1xl font-light tracking-[0.2em] text-gray-600 mb-8 font-medium uppercase">
                Projets récents
{" "}
              </h1>
            </div>
                  ...
                  return (
                    <Link
                      ...
                      className={`group cursor-pointer relative block ${
                        // Hide 4th item on medium screens and above (md+), show only 3 in a row
                        index === 3 ? "md:hidden" : ""
                      }`}
                    >
                      <div
                          ...
                          >
                            {afterImages.length === 2 && (
                              <div className="flex h-full">
                                <img
                                  src={afterImages[0]}
                                  alt={`${project.title} - After 1`}
                                  className="w-1/2 h-full object-cover"
                                />
                                <img
                                  src={afterImages[1]}
                                  alt={`${project.title} - After 2`}
                                  className="w-1/2 h-full object-cover"
                                />
                              </div>
                            )}
